...
int main( int argc, char **argv )
{
  ...
  while( fgets( line, MAX_LINE_LEN, fromServer ) ) {
    ...
    /* -------- PRE-FLOP DISCRETE ACTION LOGIC -------- */

    int mySeat = state.viewingPlayer;
    int raises = state.state.numActions[state.state.round];

    /* Sanity check: preflop only */
    assert( state.state.round == 0 );

    /* Decide action based on number of raises */
    if (raises == 0) {
        /* Open Raise */
        action.type = a_raise;
        action.size = 25;
        ...
        /* 5-bet: all-in */
        int32_t minRaise, maxRaise;

        if (raiseIsValid(game, &state.state, &minRaise, &maxRaise)) {
            action.type = a_raise;
            action.size = maxRaise;  // all-in real permitido por ACPC
            } else {
                action.type = a_call;
                action.size = 0;
            }
